...
class SocketHandler(tornado.websocket.WebSocketHandler):
    ...
    async def on_message(self, message: str):
        try:
            ...
            logging.warning("Invalid JSON syntax received from client, closing socket")
            self.close()
            return
        if not all(k in msg_data.keys() for k in ("type", "data")):
            logging.warning("Invalid data structure received from client, closing socket")
            self.close()
            return
    ...
    async def handle_message(self, msg_type: int, data):
        if msg_type == proto.MsgType.COMMAND.value:
            ...
            logging.warning("Unrecognised message type received, closing socket")
            self.close()

    async def handle_command(self, cmd: int, data):
        if cmd == proto.CmdType.START_BARCODE.value:
            logging.info("Start barcode decoding")
            ...
            logging.info("Stopping barcode decoding")
            self._barcode_running = False
            self._scanner.remove_img_queue(self._img_queue)
            self._scanner.remove_code_queue(self._code_queue)
            ...
            logging.info("Resetting compartment indicators")
            self._lights.reset()
        else:
            logging.warning("Unrecognised command received, closing socket")
            self.close()

    def handle_set_light(self, data: str, state: bool):
        logging.info(f"Setting compartment {data} {'On' if state else 'Off'}")
    ...
    def barcode_thread_f(self, server_loop):
        logging.info("Barcode image to client thread started")
        while not (self._thread_exit.is_set() or self._thread_exit_self.is_set()):
            if self._barcode_running:
                ...
                time.sleep(0.1)
        logging.info("Barcode image to client thread exiting")

    def barcode_code_thread_f(self, server_loop):
        logging.info("Barcode data to client thread started")
        while not (self._thread_exit.is_set() or self._thread_exit_self.is_set()):
            if self._barcode_running:
                try:
                ...
                time.sleep(0.1)
        logging.info("Barcode data to client thread exiting")

    def code_fetch_thread_f(self, server_loop):
        logging.info("Product lookup thread started")
        while not (self._thread_exit.is_set() or self._thread_exit_self.is_set()):
            try:
                ...
                pass
        logging.info("Product lookup thread exiting")
